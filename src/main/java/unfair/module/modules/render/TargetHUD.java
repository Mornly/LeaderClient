...
public class TargetHUD extends Module {
    ...
    public final IntProperty mixFactor = new IntProperty("MixFactor",20,1,200,() -> this.style.getValue() == 4);
    ...
    private void drawRoundedStyle(float health, float abs, float heal) {
        ...
        float headIconOffset = this.head.getValue() && this.headTexture != null ? 33.0F : 0.0F;
        float barTotalWidth = Math.max(headIconOffset + 70.0F, headIconOffset + 8.0F + barContentWidth + 8.0F);
        ...
        switch (this.posY.getValue()) {
            case 1:
                posY += (float) scaledResolution.getScaledHeight() / this.scale.getValue() / 2.0F - 20.0F;
                break;
            case 2:
                posY *= -1.0F;
                posY += (float) scaledResolution.getScaledHeight() / this.scale.getValue() - 40.0F;
        }
        GlStateManager.pushMatrix();
        ...
        RenderUtil.drawRoundedRect(0.0F, 0.0F, barTotalWidth, 40.0F, 6.0F, backgroundColor);
        float barLeft = headIconOffset + 6.0F;
        float barRight = barTotalWidth - 6.0F;
        float barTop = 28.0F;
        float barBottom = 35.0F;
        RenderUtil.drawRoundedRect(barLeft, barTop, barRight, barBottom, 3.0F, ColorUtil.darker(healthBarColor, 0.2F).getRGB());

        float healthBarWidth = lerpedHealthRatio * (barRight - barLeft);
        if (healthBarWidth > 0.0F) {
            RenderUtil.drawRoundedRect(barLeft, barTop, barLeft + healthBarWidth, barBottom, 3.0F, healthBarColor.getRGB());
        }
        ...
        if (this.head.getValue() && this.headTexture != null) {
            ...
            mc.getTextureManager().bindTexture(this.headTexture);
            Gui.drawScaledCustomSizeModalRect(5, 5, 8.0F, 8.0F, 8, 8, 28, 28, 64.0F, 64.0F);
            Gui.drawScaledCustomSizeModalRect(5, 5, 40.0F, 8.0F, 8, 8, 28, 28, 64.0F, 64.0F);
            GlStateManager.color(1.0F, 1.0F, 1.0F);
        }
